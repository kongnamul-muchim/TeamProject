...
namespace HideAndInk.Player
{
    ...
    public sealed class CamouflageAdapter : MonoBehaviour
    {
        ...
        {
            Debug.Log($"[CamouflageAdapter] TryHandleKeyDown. Current state: {_stateMachine.CurrentState}, _camouflageCooldown={_camouflageCooldown}");

            // 쿨타임 중이면 무시
            if (_camouflageCooldown > 0f)
            {
                Debug.Log("[CamouflageAdapter] Camouflage on cooldown, ignoring");
                return;
            }
