# 🐙 크레딧 구성 및 디자인 초안 (Draft)

> **프로젝트**: Hide & Ink (문어의 먹물꿈질)  
> **작성일**: 2026-05-11

---

## 1. 크레딧 텍스트 구성

팀원별 역할과 게임의 서정적인 분위기를 담은 텍스트 초안입니다.

### [ 타이틀 ]
**Hide & Ink : 문어의 먹물꿈질**

### [ STAFF ]
| 역할 (Role) | 성함 (Name) |
| :--- | :--- |
| **Lead Programmer & System Architect** | 김동열 (Dongyeol Kim) |
| **Sidekick System & Sound Sourcing** | 박시연 (Siyeon Park) |
| **Art Director & Technical Artist** | 박미초 (Micho Park) |
| **Environment Artist & Level Design** | 정지은 (Jieun Jung) |

### [ THIRD-PARTY ASSETS ]
*   **TextMesh Pro** - Unity Technologies
*   **Universal RP** - Unity Technologies
*   **Fonts** - 꾸불림체 (Kkubullim Font)

### [ MUSIC & SOUND ]

*   **Original Sound Track**
    - **Title Scene**: "Wire Wire Docks" by flowerhead
    - **Zone 1 (Reef Coast)**: "Happy Pixie Town bgm" by HitCtrl
    - **Zone 2 (Kelp Forest)**: "Dolphin Shores bgm" by flowerhead
    - **Zone 3 (Coral Reef)**: "Ruin island bgm" by AiTechEye (OpenGameArt)
    - **Zone 4 (Deep-sea Ruins)**: "ALONE.wav" by muertoapuntapies
    - **Zone 5 (Deep-sea Cliff)**: "WhaleFall OST" by TurtleBox
    - **Zone 6**: "The Last Dream" by 2LAZY / pulp_dev

*   **Sound Effects**
    - **Button Click / Stage Clear**: CLICK SFX PACK by HoveAudio
    - **Camouflage Detach**: Angrily Crumpling Piece of Paper by F.M.Audio
    - **Game Over**: Bad chest SFX by Oiboo
    - **Ink Shoot**: Propeller (Cartoon) Loop by Mish7913
    - **Predator Attack**: Water attack by xkeril

### [ SPECIAL THANKS ]
*   **Advisors**: 프로젝트에 소중한 조언을 주신 모든 분들
*   **Players**: 두두의 여정을 끝까지 지켜봐 주신 플레이어 여러분

### [ COPYRIGHT ]
© 2026 Team 미지동시. All rights reserved.  
**Powered by Unity Engine 2022.3 LTS**

---

## 2. 시각적 연출 및 디자인 컨셉

### 🎨 디자인 테마: "Emotional & Premium Indie"
*   **배경**: 완전한 블랙보다는 종이 질감이 느껴지는 어두운 다크 그레이/차콜 톤 (#1A1A1A).
*   **타이포그래피**: 
    *   색상은 순백색(#FFFFFF)보다 부드러운 미색(#F5F5F5)을 사용하여 눈의 피로를 줄임.
    *   중요 직함은 **Bold** 처리 및 자간(Letter Spacing)을 살짝 넓혀 가독성 확보.
*   **애니메이션**:
    *   **Slow Scroll**: 텍스트가 아래에서 위로 천천히 흐르는 정석적인 방식 (Speed: 50 unit/sec).
    *   **Input Response**: 클릭 또는 스페이스바 입력 시 스크롤 속도 5배 가속.

---

## 📝 팀 공유 가이드
1. **역할 표기**: 각자 표기하고 싶은 직함(Job Title)이 맞는지 최종 확인 부탁드립니다.
2. **영문 성함**: 영문 성함 표기법이 여권 등 공식 표기와 일치하는지 확인.
3. **추가 인원**: 외주 아티스트나 추가 도움 주신 분들이 있다면 섹션 추가 필요.
